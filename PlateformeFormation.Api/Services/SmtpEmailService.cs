...
public class SmtpEmailService : IEmailService
{
    ...
    public async Task SendAccountValidatedAsync(Utilisateur user, string loginUrl, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogInformation("SMTP is not configured. Skipping validation email for {Email}.", user.Email);
            return;
        }
        ...
        using var message = new MailMessage
        {
            ...
            Subject = "Votre compte a été approuvé — Plateforme de Formation",
            Body = BuildValidationHtml(user.Nom, user.Email, loginUrl),
            IsBodyHtml = true
        };
    ...
    public async Task SendAccountRejectedAsync(Utilisateur user, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(smtpHost))
        {
            _logger.LogInformation("SMTP is not configured. Skipping rejection email for {Email}.", user.Email);
            return;
        }
        ...
        using var message = new MailMessage
        {
            ...
            Body = BuildRejectionHtml(user.Nom),
            IsBodyHtml = true
        };
        message.To.Add(user.Email);
    ...
    public async Task SendPasswordResetAsync(Utilisateur user, string resetUrl, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(smtpHost))
        {
            _logger.LogInformation("SMTP is not configured. Skipping password reset email for {Email}.", user.Email);
            return;
        }
        ...
        using var message = new MailMessage
        {
            ...
            Body = BuildPasswordResetHtml(user.Nom, user.Email, resetUrl),
            IsBodyHtml = true
        };
        message.To.Add(user.Email);
...
<tr><td align=""center"">
  ...
  <table role=""presentation"" width=""100%"" cellpadding=""0"" cellspacing=""0"" style=""max-width:580px;width:100%;"">
    ...
    private static string BuildValidationHtml(string userName, string userEmail, string loginUrl) => Wrap(
    ...
    private static string BuildRejectionHtml(string userName) => Wrap(
    ...
    private static string BuildPasswordResetHtml(string userName, string userEmail, string resetUrl) => Wrap(
        headerAccentColor: "#9b111e",
        headerIcon: "&#128274;",
